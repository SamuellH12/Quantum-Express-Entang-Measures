...
from qmlHelper.metrics import squared_loss_supervised
...
TRAIN_FILENAME = "Ansatz_supervised_training_reports(CostaSH)"
EMBEDDING = ['phasex', 'phasey','amplitude']
...
METRICS = { "supervised" : squared_loss_supervised }
...
def main():
    if len(sys.argv) == 1:  # Main process
        SIZE = len(TASKS)
        DIV = 18 # int(input('How many divisions do you want? → '))
        ...
        try:
            L = int(sys.argv[1])
            R = int(sys.argv[2])
            FILENAME = f'reports_data/{TRAIN_FILENAME}_{L}_{R}.csv'
